...
def browse(message):
  ...
  bot.register_next_step_handler(current, filterSearchMethod)

# Processing user's Search Method choice. 
def filterSearchMethod(message):
  if userEnd(message) or go_back(message):
  ...
  if message.text == "Category & Keywords":
    ...
    bot.register_next_step_handler(current, searchByCatKey)    
    return
